Main website: https://laguer.github.io/Rdataset/

What is this?
=============

``Rdataset`` is a collection of over 1200 datasets that were originally distributed alongside the statistical software environment ``R`` and some of its add-on packages. The goal is to make these data more broadly accessible for teaching and statistical software development. 
...
+ `HTML index <http://github.com/laguer/Rdataset/blob/gh-pages/datasets.html>`_
+ `CSV index <http://github.com/laguer/Rdataset/blob/gh-pages/datasets.csv>`_

On the github repository you will also find: 

* ``Rdataset.R``: ``R`` script to download CSV copies and HTML docs for all datasets distributed in ``Base R`` and a list of R packages. 
